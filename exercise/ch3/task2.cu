...
__global__ void matrixVectorMultiplyKernel(const float* B, const float* C, float* A, int Width) {
...
void initialDeviceMatrix(float* B_h, float* C_h, float* A_h, int Width) {
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(B_d, B_h, matrixSize, cudaMemcpyHostToDevice));
