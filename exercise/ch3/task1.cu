 ...
 * @param Width 矩阵宽度
 */
__global__ void matrixMulElementKernel(const float* M_d, const float* N_d, float* P_d, int Width) {
    // 二维线程映射：计算当前线程对应的行和列
 ...
 * @param Width 矩阵宽度
 */
__global__ void matrixMulRowKernel(const float* M_d, const float* N_d, float* P_d, int Width) {
    // 一维线程映射：计算当前线程处理的行号
 ...
 * @param Width 矩阵宽度
 */
__global__ void matrixMulColKernel(const float* M_d, const float* N_d, float* P_d, int Width) {
    // 一维线程映射：计算当前线程处理的列号
 ...
 */
void verifyResult(const float* P_h, const float* P_ref, int Width, const char* kernelName) {
    int errorCount = 0;
 ...
 */
void matrixMulElement(const float* M_h, const float* N_h, float* P_h, int Width) {
