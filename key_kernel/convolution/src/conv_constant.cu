...
__constant__ float d_filter_constant[MAX_FILTER_SIZE * MAX_FILTER_SIZE];

__global__ void conv_constant_kernel(const float* input, float* output, int width,
...
void copy_filter_to_constant(const float* filter, int filter_size) {
    CHECK_CUDA(cudaMemcpyToSymbol(d_filter_constant, filter,
                                  filter_size * filter_size * sizeof(float), 0,
...
void launch_conv_constant(const float* d_input, float* d_output, int width, int height,
    ...
    conv_constant_kernel<<<grid, block>>>(d_input, d_output, width, height, filter_size);
