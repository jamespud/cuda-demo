...
__global__ void native_stencil_kernel(const float* __restrict__ in, float* __restrict__ out, int nx,
                                     int ny, int nz) {
    ...
    if (i > 0 && i < nx - 1 && j > 0 && j < ny - 1 && k > 0 && k < nz - 1) {
        ...
        out[idx] =
            kCenter * in[idx] + kNeighbor * (in[idx - 1] + in[idx + 1] + in[idx - nx] +
...
void launch_native(const float* d_in, float* d_out, int nx, int ny, int nz) {
    ...
    dim3 block(THREAD_PER_DIM, THREAD_PER_DIM, THREAD_PER_DIM);
    dim3 grid((nx + block.x - 1) / block.x, (ny + block.y - 1) / block.y,
              (nz + block.z - 1) / block.z);

    native_stencil_kernel<<<grid, block>>>(d_in, d_out, nx, ny, nz);
